...
export default class {
  constructor(settings, store, prefixPath) {
    ...
    this.prefixPath =
      prefixPath === ""
        ? store.getters["user/getActiveTenant"].tenant_id
        : prefixPath;
  }
  ...
  getToken(username, password) {
    ...
    return axios
      ...
      .then(data => {
        ...
        this.store.commit("auth/setAccessToken", data.token.access_token, {
          root: true
        });
        this.store.commit("auth/setRefreshToken", data.token.refresh_token, {
          root: true
        });
    ...
    if (this.isNecessaryRefresh()) {
      return axios
        ...
        .then(data => {
          ...
          this.store.commit("auth/setDecodedToken", decodedToken, {
            root: true
          });
          this.store.commit("user/setScope", decodedToken.scope, {
            root: true
          });
        });
    } else {
